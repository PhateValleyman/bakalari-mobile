...
import { schedule, weekDays, type ScheduleItem } from "@/shared/bakalari-data";
    ...
    <View style={styles.lessonRow}>
      ...
      <View className="flex-1 rounded-2xl bg-surface p-4" style={[styles.lessonCard, { borderLeftColor: item.color }]}>
        ...
        <View className="mt-3 flex-row items-center justify-between">
          <Text className="text-xs text-muted">{item.teacher}</Text>
        ...
        {item.note ? (
          <View className="mt-3 flex-row items-center gap-2 rounded-xl px-3 py-2" style={{ backgroundColor: `${item.color}12` }}>
            ...
            <Text className="text-xs font-semibold" style={{ color: item.color }}>{item.note}</Text>
          </View>
        ) : null}
      </View>
    </View>
    ...
    <ScreenContainer className="px-5" edges={["top", "left", "right"]}>
      <FlatList
        ...
        ListHeaderComponent={
          <View>
            <View className="pt-3">
              <Text className="text-sm font-semibold text-primary">Školní týden</Text>
              <View className="mt-1 flex-row items-center justify-between">
                <Text className="text-3xl font-bold text-foreground">Rozvrh</Text>
                <Pressable onPress={ping} style={({ pressed }) => [styles.iconButton, pressed && styles.pressed]}>
                  <IconSymbol name="arrow.clockwise" size={20} color={colors.primary} />
                </Pressable>
              </View>
              <Text className="mt-1 text-sm text-muted">21. 9. – 25. 9. 2026 · 3.B</Text>
            </View>
              ...
              {weekDays.map((day) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Text className="mt-1 text-base font-bold" style={{ color: active ? "#FFFFFF" : colors.foreground }}>{day.date}</Text>
                  </Pressable>
                );
              })}
            </View>
            <View className="mb-4 mt-7 flex-row items-center justify-between">
              <Text className="text-xl font-bold text-foreground">{weekDays.find((day) => day.key === selectedDay)?.label ?? "Dnes"} 25. září</Text>
              <Text className="text-sm font-semibold text-muted">{lessons.length} hodiny</Text>
            </View>
          </View>
        }
        ListEmptyComponent={<Text className="rounded-2xl bg-surface p-5 text-center text-sm text-muted">Na tento den není rozvrh.</Text>}
      />
    </ScreenContainer>
  );
}
...
const styles = StyleSheet.create({
  ...
  dayButton: { width: 54, alignItems: "center", borderRadius: 14, paddingVertical: 9 },
